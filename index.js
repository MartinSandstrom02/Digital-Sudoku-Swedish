...
function startGame() {
    ...
    lives = 3;  
    disableSelect = false;
    id("lives").textContent = "Återstående Liv: 3";
...
function pauseTimer(){
  ...
  //Display the start button and hide the paus button
    ...
    //If the number does not match the solution key
      ...
      setTimeout(function()
      {
        ...
        if(lives === 0) {endGame(); }
          ...
          //Update lives text
          id("lives").textContent = "Återstående Liv: " + lives;
...
function endGame(){
  ...
  if(lives === 0){
    id("lives").textContent = "Du förlorade!";
  qs("p#lives").classList.add("lost");
  } 
  else{
    id("lives").textContent = "Du klarade det!";
